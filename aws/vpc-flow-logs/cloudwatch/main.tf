...
resource "aws_cloudwatch_log_stream" "log_name" {
  count = var.log_destination_type == "cloud-watch-logs" ? 1 : 0
  name           = aws_cloudwatch_log_group.log_group.name[0]
  log_group_name = aws_cloudwatch_log_group.log_group.name[0]
}
...
module "enable-eni-logs" { 
  ...
  log_destination          = var.log_destination_type == "cloud-watch-logs" ? aws_cloudwatch_log_group.log_group.arn[0] : var.log_destination 
