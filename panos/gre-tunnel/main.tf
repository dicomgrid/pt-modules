...
#   name               = var.tunnel_interface.name
...
resource "panos_gre_tunnel" "main" {
  ...
  tunnel_interface      = var.tunnel_interface #try(panos_tunnel_interface.main[0].name, var.tunnel_interface.name, var.tunnel_interface)
  ttl                   = var.ttl
